...
router.get('/ids', async (req, res) => {
  try {
    ...
    res.json(
      data
        ? { success: true, data }
        : { success: false, message: 'No se encontraron IDs' }
    );
  } catch (error) {
    console.error(error);
...
router.get('/registro_reservations', async (req, res) => {
  ...
  try {
    const data = await Reserva.obtenerreservationsPorID(id);
    res.json(
      data
        ? { success: true, data }
    ...
    console.error(error);
    res.status(500).json({ error: 'Error al obtener las reservations' });
  }
});
