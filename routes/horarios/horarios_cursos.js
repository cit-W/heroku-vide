...
router.post('/crear_horario', async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: 'Error al registrar el trabajo social' });
  }
});
...
router.get('/crear_tabla_cursos', async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: 1,
      message: `La tabla '${curso}' se creó correctamente.`,
    });
  } catch (err) {
    console.error(err);
    // Respuesta de error
    res
      .status(500)
      .json({ success: 0, error: `Error al crear la tabla: ${err.message}` });
  }
});
...
router.post('/delete_horario/:id', async (req, res) => {
  ...
  if (!name) {
    return res
      .status(400)
      .json({ success: false, error: 'No se proporcionó un cedula válido' });
  }
  ...
  try {
    ...
    res.json({
      success: true,
      message: `Tabla '${name}' eliminada exitosamente`,
    });
  } catch (err) {
    console.error(err);
...
router.get('/registro_horario_account', async (req, res) => {
  ...
  if (!nameCorrect) {
    return res
      .status(400)
      .json({ success: false, error: 'No se proporcionó un nombre válido' });
  }
